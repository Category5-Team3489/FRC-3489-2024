...
public class RobotContainer {
        ...
        private final SetShooterAngle setShooterAngle = new SetShooterAngle(shooterAngle, () -> {
                return manipulatorXbox.getLeftY();
        });
        private final SetShooterSpeed stopShooter = new SetShooterSpeed(() -> {
                return 0.0;
        }, shooterSpeed);
       
        private final ManualShoot setShooterFar = new ManualShoot(Constants.ShooterSpeed.FarShooterSpeed, shooterSpeed, shooterAngle, Constants.ShooterAngle.FarShooterAngle, index);
        private final ManualShoot setShooterClose = new ManualShoot(Constants.ShooterSpeed.CloseShooterSpeed, shooterSpeed, shooterAngle, Constants.ShooterAngle.CloseShooterAngle, index);
        ...
        private final SwerveRequest.FieldCentric drive = new SwerveRequest.FieldCentric()
                        .withDeadband(MaxMetersPerSecond * 0.1).withRotationalDeadband(MaxRadiansPerSecond * 0.1) // Add
                                                                                                                  // a
                                                                                                                  // 10%
                                                                                                                  // deadband
        ...
        private void configureBindings() {
                ...
                drivetrain.setDefaultCommand( // Drivetrain will execute this command periodically
                                drivetrain.applyRequest(() -> drive
                                                .withVelocityX(-driverXbox.getLeftY() * MaxMetersPerSecond) // Drive forward with negative Y (forward)
                                                .withVelocityY(-driverXbox.getLeftX() * MaxMetersPerSecond) // Drive left with
                                                                                                            // negative
                                                                                                            // X (left)
                                                                                                            // 
                                                .withRotationalRate(-driverXbox.getRightX() * MaxRadiansPerSecond) // Drive counterclockwise with negative X (left)
                                ));
                ...
                //Manual Shoot
                manipulatorXbox.b().onTrue(index.indexCommand(IndexState.BeltShooter));

                // TODO See fieldRelative
                
                // TODO right joystick = shooter angle
                ...
                manipulatorXbox.rightBumper().onTrue(setShooterFar);

                // TODO a = auto shoot
                manipulatorXbox.a().whileTrue(shooterSpeed.shootNote());

                // x = stop shooter;
                manipulatorXbox.x().onTrue(stopShooter);

                // y = intake
                manipulatorXbox.y().onTrue(intakeUntilDetection);
                
                // right Trigger and y = outtake
                manipulatorXbox.rightTrigger().and(manipulatorXbox.y()).onTrue(intake.intakeCommand(IntakeState.Out));
